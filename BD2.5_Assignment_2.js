...
function getStocksSortedByPriceLowToHigh(stock1, stock2) {
  return stock1.price - stock2.price;
}
function getStocksSortedByPriceHighToLow(stock1, stock2) {
  return stock2.price - stock1.price;
}
app.get("/stocks/sort/pricing", (req, res) => {
  let pricing = req.query.pricing;
  let sortedStocks = stocks.slice();
  if (pricing.toLocaleLowerCase() === "low-to-high") {
    sortedStocks.sort(getStocksSortedByPriceLowToHigh);
  } else if (pricing.toLocaleLowerCase() === "high-to-low") {
    sortedStocks.sort(getStocksSortedByPriceHighToLow);
  }
  res.json({ stocks: sortedStocks });
});

// Endpoint 2: Get the stocks sorted based on their Growth.
function getStocksSortedByGrowthLowToHigh(stock1, stock2) {
  return stock1.growth - stock2.growth;
}
function getStocksSortedByGrowthHighToLow(stock1, stock2) {
  return stock2.growth - stock1.growth;
}
app.get("/stocks/sort/growth", (req, res) => {
  let growth = req.query.growth;
  let sortedStocks = stocks.slice();
  if (growth.toLocaleLowerCase() === "low-to-high") {
    sortedStocks.sort(getStocksSortedByGrowthLowToHigh);
  } else if (growth.toLocaleLowerCase() === "high-to-low") {
    sortedStocks.sort(getStocksSortedByGrowthHighToLow);
  }
  res.json({ stocks: sortedStocks });
});
