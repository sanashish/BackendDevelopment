...
function getHotelsSortedByPricingHighToLow(hotel1, hotel2) {
  return hotel2.price - hotel1.price;
}
function getHotelsSortedByPricingLowToHigh(hotel1, hotel2) {
...
app.get("/hotels/sort/pricing", (req, res) => {
  ...
  if (pricing.toLowerCase() === "low-to-high") {
    result.sort(getHotelsSortedByPricingLowToHigh);
  } else if (pricing.toLowerCase() === "high-to-low") {
    result.sort(getHotelsSortedByPricingHighToLow);
  }
  res.json({ hotels: result });
});

// Endpoint 2: Get the hotels sorted based on their Ratings
function getHotelsSortedByRatingsHighToLow(hotel1, hotel2) {
  return hotel2.rating - hotel1.rating;
}
function getHotelsSortedByRatingsLowToHigh(hotel1, hotel2) {
...
app.get("/hotels/sort/rating", (req, res) => {
  ...
  if (rating.toLowerCase() === "low-to-high") {
    result.sort(getHotelsSortedByRatingsLowToHigh);
  } else if (rating.toLowerCase() === "high-to-low") {
    result.sort(getHotelsSortedByRatingsHighToLow);
  }
  res.json({ hotels: result });
});

// Endpoint 3: Get the Hotels sorted based on their Reviews
function getHotelsSortedByReviewsMostToLeast(hotel1, hotel2) {
  return hotel2.reviews - hotel1.reviews;
}
function getHotelesSortedByReviewsLeastToMost(hotel1, hotel2) {
...
app.get("/hotels/sort/reviews", (req, res) => {
  ...
  if (reviews.toLowerCase() === "least-to-most") {
    result.sort(getHotelesSortedByReviewsLeastToMost);
  } else if (reviews.toLowerCase() === "most-to-least") {
    result.sort(getHotelsSortedByReviewsMostToLeast);
  }
  res.json({ hotels: result });
});
